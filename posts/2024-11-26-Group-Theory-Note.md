...
If $H$ contains infinite many $a_i$, then $H = G$.
If $H$ contains only finite many $a_i$, say $\{
    a_{i_1}, a_{i_2}, \dots, a_{i_n}
\}$ such that $i_1 < i_2 < \dots < i_n$.
Then by previous argument, $a_{i_1}$, $a_{i_2}$, $\dots$, $a_{i_{n-1}}$
can be generated by $a_{i_n}$.
Thus, $H$ is a subgroup of $\langle a_{i_n}\rangle$,
which is finite and cyclic.
As $H$ is a subgroup of finite and cyclic group, $H$ is finite and cyclic.
