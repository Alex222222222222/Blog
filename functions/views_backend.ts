...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  let reportRequestBodyString = JSON.stringify(reportRequestBody);

  if (path.length === 0) {
    const reportRequestBodyNew = {
      dimensions: [
        {
          name: "pagePath",
        },
    ...
    reportRequestBodyString = JSON.stringify(reportRequestBodyNew);
  }
